...
protected:
	cocos2d::Sprite* _bg;         ///<底盘的精灵
	cocos2d::Sprite* _handle;     ///<摇杆的精灵

	float _bgRadius;              ///<底盘的半径
	float _handleRadius;          ///<摇杆的半径
	bool  _bMove;		           ///<摇杆是否正在移动

	cocos2d::Vec2 _handlePos;     ///<摇杆在底盘坐标系的坐标
	
	JoystickEventCallback _touchEventCallback;
	TouchOutsideHandleEvent _touchOutsideCallback;
...
{
	_touchOutsideCallback = callback;
}
